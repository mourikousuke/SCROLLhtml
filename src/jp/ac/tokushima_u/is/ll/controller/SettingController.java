...
//    		userService.addmember(userid.getId(),authorid,groupname);
   	...
   	public String quizset(@ModelAttribute("setting") SettingForm form,
   			ModelMap model, HttpServletRequest request,String authorid,String groupname) {
//    	   String groupname="";
    	   Usertest userid = userService.findById(SecurityUserHolder
     				.getCurrentUser().getId());
    	   	LoginConfirm setting=userService.getmyquiz(userid.getId());
           	if(setting==null){
        	userService.setmyquiz(userid.getId(),form.getD1(),form.getD2());
           	}
           	else{
           		userService.updatemyquiz(userid.getId(),form.getD1(),form.getD2());
           	}
       ...
       @RequestMapping(value = "/quizdelete", method = RequestMethod.POST)
      	public String quizset2(@ModelAttribute("setting") SettingForm form,
      			ModelMap model, HttpServletRequest request,String authorid,String groupname) {
//       	   String groupname="";
       	   Usertest userid = userService.findById(SecurityUserHolder
        				.getCurrentUser().getId());
       	   	userService.deletequiz(userid.getId());
       ...
       @RequestMapping(value = "/mygoal", method = RequestMethod.POST)
      	public String mygoal(@ModelAttribute("setting") SettingForm form,
      			ModelMap model, HttpServletRequest request,String authorid,String groupname) {
//       	   String groupname="";
       	   Usertest userid = userService.findById(SecurityUserHolder
        				.getCurrentUser().getId());
       	  ...
       	  int a =Integer.parseInt(form.getG_totalword())/Integer.parseInt(form.getG_totaltime());
       	  form.setG_oneword(String.valueOf(a));
       	  
       	LoginConfirm setting=userService.getmygoal(userid.getId());
       	if(setting==null){
    	userService.setgoal(userid.getId(),form.getN1(),form.getG_totaltime(),form.getG_onetime(),form.getG_totalword(),form.getG_oneword(),currenttime);
       	}
       	else{
       		userService.updategoal(userid.getId(),form.getN1(),form.getG_totaltime(),form.getG_onetime(),form.getG_totalword(),form.getG_oneword(),currenttime);
       	}
