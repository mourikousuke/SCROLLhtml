...
public class QuizController {
	...
	public String mq(@ModelAttribute("form") QuizCondition quizCon,
			ModelMap model, String lat, String lng, String season,
			String timezone, String place) throws ParseException {
		model.clear();
	...
	public String mysave(String quizid, String itemid, String place,
			String placeattribute, String answerstate, String answertype,
			ModelMap model) throws ParseException {
		//
		Users user = userService.getById(SecurityUserHolder.getCurrentUser()
				.getId());
		Date nowdate = new Date();
		itemService.setquizans(KeyGenerateUtil.generateIdUUID(), quizid,
				itemid, user.getId(), place, nowdate, answerstate, answertype);

		return "quiz/myquiz";
	}

	@RequestMapping(value = "/quiztest", method = RequestMethod.GET)
	public String test(String jp, String place, String time, ModelMap model)
			throws ParseException {
		//
		Users user = userService.getById(SecurityUserHolder.getCurrentUser()
				.getId());

		LoginConfirm realuser = itemService.getrealuser(user.getId());
		...
		String real_jplevel = "";
		String real_goal = jp;
		real_goal = real_goal.replaceAll("n", "N");
		String real_place = place;
		String real_time = time;
		// 類似性
		if (realuser != null) {
			if (realuser.getAge() != null) {
				real_age = realuser.getAge();
			} else {
				real_age = "";
			}
			if (realuser.getGender() != null) {
				real_gender = realuser.getGender();
			} else {
				real_gender = "";
			}
			if (realuser.getNationality() != null) {
				real_native = realuser.getNationality();
			} else {
				real_native = "";
			}
			if (realuser.getStay() != null) {
				real_stay = realuser.getStay();
			} else {
				real_stay = "";
			}
			if (realuser.getJ_level() != null) {
				real_jplevel = realuser.getJ_level();
			} else {
				real_jplevel = "";
			}
		...
		List<LoginConfirm> cyber_data = new ArrayList<LoginConfirm>();
		cyber_data = itemService.getCyberdata(real_goal);
		// List<Integer> v_real =new ArrayList<Integer>();
		// List<Integer> v_cyber =new ArrayList<Integer>();
		List<Integer> vector = new ArrayList<Integer>();
		List<LoginConfirm> recommend = new ArrayList<LoginConfirm>();

		for (int i = 0; i < cyber_data.size(); i++) {
			List<Integer> v_real = new ArrayList<Integer>();
			List<Integer> v_cyber = new ArrayList<Integer>();
			String diff_age = "";
			String diff_gender = "";
			...
			String diff_level = "";
			String diff_place = "";
			if (cyber_data.get(i).getAge() != null) {
				if (cyber_data.get(i).getAge().equals(real_age)) {
					v_real.add(1);
					v_cyber.add(1);
				} else {
					diff_age = cyber_data.get(i).getAge();
					v_real.add(1);
					v_cyber.add(0);
					v_real.add(0);
					v_cyber.add(1);
			...
			if (cyber_data.get(i).getGender() != null) {
				if (cyber_data.get(i).getGender().equals(real_gender)) {
					v_real.add(1);
					v_cyber.add(1);
				} else {
					diff_gender = cyber_data.get(i).getGender();
					v_real.add(1);
					v_cyber.add(0);
					v_real.add(0);
					v_cyber.add(1);
			...
			if (cyber_data.get(i).getNationality() != null) {
				if (cyber_data.get(i).getNationality().equals(real_native)) {
					v_real.add(1);
					v_cyber.add(1);
				} else {
					diff_native = cyber_data.get(i).getNationality();
					v_real.add(1);
					v_cyber.add(0);
					v_real.add(0);
					v_cyber.add(1);
			...
			if (cyber_data.get(i).getStay() != null) {
				if (cyber_data.get(i).getStay().equals(real_stay)) {
					v_real.add(1);
					v_cyber.add(1);
				} else {
					v_real.add(1);
					...
					v_real.add(0);
					v_cyber.add(1);
					diff_stay = cyber_data.get(i).getStay();
				}
			} else {
				v_cyber.add(0);
				v_real.add(0);
			...
			if (cyber_data.get(i).getJ_level() != null) {
				if (cyber_data.get(i).getJ_level().equals(real_goal)) {
					v_real.add(1);
					v_cyber.add(1);
				} else {
					v_real.add(1);
					...
					v_real.add(0);
					v_cyber.add(1);
					diff_level = cyber_data.get(i).getJ_level();
				}
			} else {
				v_cyber.add(0);
				v_real.add(0);
			}
			if (cyber_data.get(i).getPlaceattribute() != null) {
				if (cyber_data.get(i).getPlaceattribute().equals(real_place)) {
					v_real.add(1);
					v_cyber.add(1);
				} else {
					v_real.add(1);
					...
					v_real.add(0);
					v_cyber.add(1);
					diff_place = cyber_data.get(i).getPlaceattribute();
				}
			} else {
			...
			int A_sq = 0;
			int B_sq = 0;
			for (int j = 0; j < v_real.size(); j++) {
				A = A + (v_real.get(j) * v_cyber.get(j));
			}

			for (int j = 0; j < v_real.size(); j++) {
				A_sq = A_sq + (v_real.get(j) * v_real.get(j));

			}
			for (int j = 0; j < v_real.size(); j++) {
				B_sq = B_sq + (v_cyber.get(j) * v_cyber.get(j));
			...
			//
			double consine = A / (a * b);
			// System.out.println("cos類似度" + consine);
			LoginConfirm log = new LoginConfirm();
			log.setItemid(cyber_data.get(i).getItemid());
			log.setContent(cyber_data.get(i).getContent());
			log.setKana(cyber_data.get(i).getKana());
			log.setKanji(cyber_data.get(i).getKanji());
			log.setImage(cyber_data.get(i).getImage());
			log.setCos(consine);
			recommend.add(log);
		}

		Collections.sort(recommend, new Comparator<LoginConfirm>() {
			public int compare(LoginConfirm t1, LoginConfirm t2) {
				return Double.compare(t1.getCos(), t2.getCos());
				// +(int)((double) t2.getCos() - (double)t1.getCos());
				// return
		...
		for (int i = 0; i < recommend.size(); i++) {
			System.out.println("consine" + recommend.get(i).getCos());
			System.out.println("itemid" + recommend.get(i).getItemid());
			System.out.println("Content" + recommend.get(i).getContent());
		}

		Quizmeansuregenerator(recommend, model);
		return "quiz/test";
	}

	@RequestMapping(value = "/locset", method = RequestMethod.GET)
	public String locset(@ModelAttribute("setting") SettingForm form,
	...
	private void Quizmeansuregenerator(List<LoginConfirm> recommend,
			ModelMap model) {
		// TODO Auto-generated method stub
		Random quizchoice = new Random();
		int quiztype = quizchoice.nextInt(5);
		int quiztype2 = quizchoice.nextInt(5) + 5;
		int quiztype3 = quizchoice.nextInt(5) + 10;
		int quiztype4 = quizchoice.nextInt(5) + 15;
		int quiztype5 = quizchoice.nextInt(5) + 20;

		int randomselect = quizchoice.nextInt(50) + 100;
		int re_i = 0;
		List<Mychoice> selectquiz1 = new ArrayList<Mychoice>();
		List<Mychoice> selectquiz2 = new ArrayList<Mychoice>();
		List<Mychoice> selectquiz3 = new ArrayList<Mychoice>();
		List<Mychoice> selectquiz4 = new ArrayList<Mychoice>();
		List<Mychoice> selectquiz5 = new ArrayList<Mychoice>();
		...
		for (int i = 1; i < 4; i++) {
			...
			if (itemService
					.getquizmean(recommend.get(quiztype + i).getItemid()) != null) {
				mc.setMycontent(itemService.getquizmean(
						recommend.get(quiztype + i).getItemid()).getContent());
				mc.setAnswer("0");
				mc.setContent(recommend.get(quiztype + i).getContent());

			} else {
				int ran = quizchoice.nextInt(5);
				if(ran==1){
					mc.setMycontent("test");
					mc.setAnswer("0");
					mc.setContent("テスト");	
				}
				else if(ran==2){
					mc.setMycontent("physical");
					mc.setAnswer("0");
					mc.setContent("フィジカル");
				}
				else if(ran==3){
					mc.setMycontent("nonsense");
					mc.setAnswer("0");
					mc.setContent("ナンセンス");
				}
				else{
					mc.setMycontent("naive");
					mc.setAnswer("0");
					mc.setContent("ナイーブ");
				}
		...
		mc = new Mychoice();
		if (itemService.getquizmean(recommend.get(quiztype).getItemid()) != null) {
			mc.setMycontent(itemService.getquizmean(
					recommend.get(quiztype).getItemid()).getContent());
			mc.setAnswer("1");
			mc.setContent(recommend.get(quiztype).getContent());

		} else {
			int ran = quizchoice.nextInt(5);
			if(ran==1){
				mc.setMycontent("motivation");
				mc.setAnswer("1");
				mc.setContent("モチベーション");	
			}
			else if(ran==2){
				mc.setMycontent("Logic");
				mc.setAnswer("1");
				mc.setContent("ロジック");
			}
			else if(ran==3){
				mc.setMycontent("clever");
				mc.setAnswer("1");
				mc.setContent("賢い");
			}
			else{
				mc.setMycontent("Tablet");
				mc.setAnswer("1");
				mc.setContent("タブレット");
			}
		...
		selectquiz1.add(mc);
		Collections.shuffle(selectquiz1);
		model.addAttribute("quiz1", selectquiz1);

		// quiz2
		for (int i = 1; i < 4; i++) {
			mc = new Mychoice();
			if (itemService.getquizmean(recommend.get(quiztype2 + i)
					.getItemid()) != null) {

				mc.setMycontent(itemService.getquizmean(
						recommend.get(quiztype2 + i).getItemid()).getContent());
				mc.setAnswer("0");
				mc.setContent(recommend.get(quiztype2 + i).getContent());

			} else {
				int ran = quizchoice.nextInt(5);
				if(ran==1){
					mc.setMycontent("Process");
					mc.setAnswer("0");
					mc.setContent("プロセス");	
				}
				else if(ran==2){
					...
					mc.setMycontent("Desktop");
					mc.setAnswer("0");
					mc.setContent("デスクトップ");
				}
		...
		mc = new Mychoice();
		if (itemService.getquizmean(recommend.get(quiztype2).getItemid()) != null) {
			mc.setMycontent(itemService.getquizmean(
					recommend.get(quiztype2).getItemid()).getContent());
			mc.setAnswer("1");
			mc.setContent(recommend.get(quiztype2).getContent());
			selectquiz2.add(mc);
		} else {
			mc.setMycontent("Ubiquitous");
			mc.setAnswer("1");
			mc.setContent("ユビキタス");
			selectquiz2.add(mc);

		}
		Collections.shuffle(selectquiz2);
		model.addAttribute("quiz2", selectquiz2);

		// quiz3
		for (int i = 1; i < 4; i++) {
			mc = new Mychoice();
			if (itemService.getquizmean(recommend.get(quiztype3 + i)
					.getItemid()) != null) {
				mc.setMycontent(itemService.getquizmean(
						recommend.get(quiztype3 + i).getItemid()).getContent());
				mc.setAnswer("0");
				mc.setContent(recommend.get(quiztype3 + i).getContent());
				selectquiz3.add(mc);

			} else {
				int ran = quizchoice.nextInt(5);
				if(ran==1){
					mc.setMycontent("Information");
					mc.setAnswer("0");
					mc.setContent("情報");	
				}
				else if(ran==2){
					mc.setMycontent("SCROLL");
					mc.setAnswer("0");
					mc.setContent("スクロール");
				}
				else if(ran==3){
					mc.setMycontent("Next");
					mc.setAnswer("0");
					mc.setContent("次に");
				}
				else{
					mc.setMycontent("Between");
					mc.setAnswer("0");
					mc.setContent("間");
				}
				selectquiz3.add(mc);
		...
		mc = new Mychoice();
		if (itemService.getquizmean(recommend.get(quiztype3).getItemid()) != null) {
			mc.setMycontent(itemService.getquizmean(
					recommend.get(quiztype3).getItemid()).getContent());
			mc.setAnswer("1");
			mc.setContent(recommend.get(quiztype3).getContent());
			selectquiz3.add(mc);
			...
			int ran = quizchoice.nextInt(5);
			if(ran==1){
				mc.setMycontent("Learning Log");
				mc.setAnswer("1");
				mc.setContent("ラーニングログ");	
			}
			else if(ran==2){
				mc.setMycontent("fan");
				mc.setAnswer("1");
				...
				mc.setMycontent("Learning Analytics");
				mc.setAnswer("1");
				mc.setContent("学習分析論");
			}
			else{
				mc.setMycontent("Place");
				mc.setAnswer("1");
				mc.setContent("場所");
			}
			selectquiz3.add(mc);
		...
		Collections.shuffle(selectquiz3);
		model.addAttribute("quiz3", selectquiz3);

		// quiz4
				for (int i = 1; i < 4; i++) {
					mc = new Mychoice();
					if (itemService.getquizmean(recommend.get(quiztype4 + i)
							.getItemid()) != null) {
						mc.setMycontent(itemService.getquizmean(
								recommend.get(quiztype4 + i).getItemid()).getContent());
						mc.setAnswer("0");
						mc.setContent(recommend.get(quiztype4 + i).getContent());
						selectquiz4.add(mc);

					} else {
						int ran = quizchoice.nextInt(5);
						if(ran==1){
							mc.setMycontent("Google");
							mc.setAnswer("0");
							mc.setContent("グーグル");	
						}
						else if(ran==2){
							mc.setMycontent("Yahoo");
							mc.setAnswer("0");
							mc.setContent("ヤフー");
						}
						else if(ran==3){
							mc.setMycontent("Apple");
							mc.setAnswer("0");
							mc.setContent("アップル");
						}
						else{
							mc.setMycontent("Cyber");
							mc.setAnswer("0");
							mc.setContent("サイバー");
						}
						selectquiz4.add(mc);
				...
				mc = new Mychoice();
				if (itemService.getquizmean(recommend.get(quiztype4).getItemid()) != null) {
					mc.setMycontent(itemService.getquizmean(
							recommend.get(quiztype4).getItemid()).getContent());
					mc.setAnswer("1");
					mc.setContent(recommend.get(quiztype4).getContent());
					selectquiz4.add(mc);
					...
					int ran = quizchoice.nextInt(5);
					if(ran==1){
						mc.setMycontent("Orange");
						mc.setAnswer("1");
						mc.setContent("オレンジ");	
					}
					else if(ran==2){
						mc.setMycontent("Bike");
						mc.setAnswer("1");
						mc.setContent("バイク");
					}
					else if(ran==3){
						mc.setMycontent("Anime");
						mc.setAnswer("1");
						mc.setContent("アニメ");
					}
					else{
						mc.setMycontent("Location");
						mc.setAnswer("1");
						mc.setContent("ローケーション");
					}
					selectquiz4.add(mc);

				}
				Collections.shuffle(selectquiz4);
				model.addAttribute("quiz4", selectquiz4);

				// quiz5
				for (int i = 1; i < 4; i++) {
					mc = new Mychoice();
					if (itemService.getquizmean(recommend.get(quiztype5 + i)
							.getItemid()) != null) {
						mc.setMycontent(itemService.getquizmean(
								recommend.get(quiztype5 + i).getItemid()).getContent());
						mc.setAnswer("0");
						mc.setContent(recommend.get(quiztype5 + i).getContent());
						selectquiz5.add(mc);

					} else {
						int ran = quizchoice.nextInt(5);
						if(ran==1){
							mc.setMycontent("Cola");
							mc.setAnswer("0");
							mc.setContent("コーラー");	
						}
						else if(ran==2){
							mc.setMycontent("Spring");
							mc.setAnswer("0");
							mc.setContent("春");
						}
						else if(ran==3){
							mc.setMycontent("Summer");
							mc.setAnswer("0");
							mc.setContent("夏");
						}
						else{
							mc.setMycontent("Winter");
							mc.setAnswer("0");
							mc.setContent("冬");
						}
						selectquiz5.add(mc);
				...
				mc = new Mychoice();
				if (itemService.getquizmean(recommend.get(quiztype5).getItemid()) != null) {
					mc.setMycontent(itemService.getquizmean(
							recommend.get(quiztype5).getItemid()).getContent());
					mc.setAnswer("1");
					mc.setContent(recommend.get(quiztype5).getContent());
					selectquiz5.add(mc);
					...
					int ran = quizchoice.nextInt(5);
					if(ran==1){
						mc.setMycontent("Visualization");
						mc.setAnswer("1");
						mc.setContent("可視化");	
					}
					else if(ran==2){
						mc.setMycontent("Workshop");
						mc.setAnswer("1");
						mc.setContent("ワークショップ");
					}
					else if(ran==3){
						mc.setMycontent("Combination");
						mc.setAnswer("1");
						mc.setContent("コンビネーション");
					}
					else{
						mc.setMycontent("Review");
						mc.setAnswer("1");
						mc.setContent("レビュー");
					}
					selectquiz5.add(mc);
				...
				Collections.shuffle(selectquiz5);
				model.addAttribute("quiz5", selectquiz5);
		...
		// mc = new Mychoice();
		// mc.setMycontent(itemService.getquizmean(
		// recommend.get(quiztype + 4).getItemid()).getContent());
		// mc.setAnswer("0");
		// mc.setContent(recommend.get(quiztype4 + i).getContent());
		// selectquiz4.add(mc);
		//
	...
	private void Quizautogenerate(Users user, ModelMap model)
		...
		Usertest userid = userService.findById(SecurityUserHolder
				.getCurrentUser().getId());
		LoginConfirm timequiz = userService.getmyquiz(userid.getId());
		if (timequiz == null) {
			// yes no quiz
			if (quiztype <= 3) {
				...
				if (gquiz.size() != 0) {
					...
					// 自分の母国語
					if (results.size() <= 0) {
						results = itemService.question2(mc.getAuthorid(),
								mc.getItemid());
						if (results.size() <= 0) {
							mc.setMycontent("");
						} else {
					...
					// 補助教材
					List<WordNet> wordnet_sentence = itemService
							.getwordnet(gquiz.get(0).getContent());
			...
			// image multiple choice quiz
				...
				if (gquiz.size() >= 4) {
					...
					for (int i = 0; i < 4; i++) {
						...
						selectquiz.add(mc);
						// common id update
						itemService.updateCommonid(gquiz.get(i).getId(),
								commonid);
			...
			// text multiple choice quiz
				...
				if (gquiz.size() >= 4) {
					...
					itemService.updatequizdate(cdate2);
					List<Results> results2 = new ArrayList<Results>();
					List<MqQuiz> gquiz2 = itemService.getimagegquiz(user
							.getId());
					List<Mychoice> selectquiz2 = new ArrayList<Mychoice>();
					if (gquiz2.size() >= 4) {
						String commonid = KeyGenerateUtil.generateIdUUID();
						for (int i = 0; i < 4; i++) {
							...
							itemService.updatemqquiz(quizstore);
							itemService.insertmqc(mc);
							results2 = itemService.question(mc.getAuthorid(),
									mc.getItemid());

							if (results2.size() <= 0) {
								results2 = itemService.question2(
										mc.getAuthorid(), mc.getItemid());
								if (results2.size() <= 0) {
									mc.setMycontent("");
								} else {
									mc.setMycontent(results2.get(0)
											.getContent());
								}
							} else {
								mc.setMycontent(results2.get(0).getContent());
							}
							// mc.setMycontent(results.get(0).getContent());
		...
		// *************************timequiz***********************************************
			...
			if (quiztype <= 4) {
				...
				Date d1 = sdf.parse(timequiz.getDatef());
				Date d2 = sdf.parse(timequiz.getDatee());
				List<MqQuiz> gquiz = itemService.getgquiztime(user.getId(), d1,
						d2);
				List<Mychoice> selectquiz = new ArrayList<Mychoice>();
				List<Results> results = new ArrayList<Results>();
				if (gquiz.size() != 0) {
	...
	public String setting(@ModelAttribute("form") AnalysisForm form,
			ModelMap model, HttpServletRequest request) {
		// Users user = userService.getById(SecurityUserHolder.getCurrentUser()
		// .getId());
		// List<Quizanalysis> quizdata = this.quizanalysisservice
